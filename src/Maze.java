...
public class Maze {
     ...
     *            - the starting vertex
     * @param endVertex
     *            - we will stop the traversal and output the list of moves when we
     *            hit this vertex
     * 
     */
	...
	dfsAlgorithmSearch(startVertex, visited, moveList);
	return moveList;
    }

    private boolean dfsAlgorithmSearch(int v, boolean[] visited, LinkedList<Move> moveList) {
	visited[v] = true;
	if (v == endVertex) {
	    return true;
	}
	...
	while (neighborIte.hasNext()) {
		...
		if (dfsAlgorithmSearch(neighbor, visited, moveList)) {
     ...
     * 
     * @param filename
     *            - the name of the file to be read from, containing the maze data
     */
